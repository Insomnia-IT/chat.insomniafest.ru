...
async def require_hr(update: Update) -> bool:
    ...
    await update.effective_message.reply_text("❌ Эта команда недоступна.")
    return False
...
async def ops_sync(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not ok:
        await update.effective_message.reply_text("❌ Не удалось обновить кэш Гриста.")
        return
...
async def _hr_require_eligible(
    ...
    if not check_ok:
        ...
        return None
    if not eligible:
        await update.effective_message.reply_text(f"❌ Участник не найден: {handle}")
        return None
...
async def ops_check(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.effective_message.reply_text("Использование: /hr_check <телеграм_ник>")
        return
...
async def ops_register(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.effective_message.reply_text("Использование: /hr_register <телеграм_ник>")
        return
    ...
    if created:
        user_message = "\n".join([
            ...
            "Если потеряешь логин или пароль, их можно получить через бота или у HR.",
            "Участие в таблице не обязательно для входа в чат, но нужно для автоматического добавления в командный чат.",
